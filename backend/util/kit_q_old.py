...
def init_queue_db():
    with _db_lock:
        ...
        try:
            ...
            # Recover jobs that were interrupted by a Raspberry Pi reboot/app reset.
            # Before capture is saved, the safest action is to run the queue job again.
            conn.execute(
                """
                UPDATE kit_queue
                SET status = 'queued',
                    updated_at = ?,
                    last_error = 'Recovered after shutdown before capture completed.'
                WHERE status IN ('processing', 'capturing', 'analyzing')
                """,
                (now_text,),
            )

            # If the app stopped while uploading, keep the saved capture and retry upload.
            conn.execute(
                """
                UPDATE kit_queue
                SET status = 'upload_pending',
                    updated_at = ?,
                    last_error = 'Recovered after shutdown during upload. Upload will retry.'
                WHERE status = 'uploading'
                """,
                (now_text,),
            )

            # If the app stopped while disposal was waiting for Arduino confirmation,
            # do not retry the trash motor automatically. Keep the captured images and
            # retry website upload/resync so the transaction is still visible online.
            conn.execute(
                """
                UPDATE kit_queue
                SET status = 'disposal_failed_upload_pending',
                    updated_at = ?,
                    last_error = 'Recovered after shutdown during trash disposal. Operator inspection is required; upload will retry.'
                WHERE status = 'disposing'
                  AND (
                    COALESCE(raw_path, '') != ''
                    OR COALESCE(annotated_path, '') != ''
                    OR COALESCE(result, '') != ''
                  )
                """,
                (now_text,),
            ...
            conn.execute(
                ...
                SET status = 'queued',
                    updated_at = ?,
                    last_error = 'Recovered after shutdown during disposal before capture data was saved.'
                WHERE status = 'disposing'
                """,
                (now_text,),
            )
    ...
    with _db_lock:
        ...
        try:
            row = conn.execute(
                ...
                SELECT *
                FROM kit_queue
                WHERE status IN ('upload_pending', 'disposal_failed_upload_pending')
...
class KitQueueWorker:
    ...
    def worker_loop(self):
        while not self.stop_event.is_set():
            try:
                # Physical disposal/trash movement is not retried automatically.
                # If disposal fails, the error remains visible/logged for operator action.
    ...
    def process_due_job(self, job):
        ...
        try:
            ...
            if not dispose_ok:
                # Critical fix: do not let an Arduino/trash timeout prevent the
                # saved capture/result from reaching the website. The trash motor
                # is still NOT retried automatically; only website upload is retried.
                if upload_ok:
                    latest_after_upload = get_job(job_id) or latest
                    disposal_message = latest_after_upload.get("last_error") or "Trash disposal failed. Operator assistance is required."
                    mark_job(job_id, "disposal_failed", last_error=disposal_message)
                    publish_queue_state(
                        "disposal_failed",
                        "Result uploaded, but trash disposal needs operator assistance.",
                    )
                    print(
                        f"[KIT QUEUE] Upload completed despite disposal failure job={job_id}",
                        flush=True,
                    )
                else:
                    mark_job(
                        job_id,
                        "disposal_failed_upload_pending",
                        last_error="Trash disposal failed and website upload is pending. Operator assistance is required.",
                    )
                    publish_queue_state(
                        "disposal_failed_upload_pending",
                        f"Disposal failed; upload pending for queued kit {job_id}",
                    )
                    print(
                        f"[KIT QUEUE] Disposal failed and upload pending job={job_id}",
                        flush=True,
                    )
                return
    ...
    def dispose_job(self, job_id: str):
        ...
        if not result.get("success"):
            ...
            report_error(
                ...
                "The processed kit could not be moved to the trash bin. Operator assistance is required. The booth will not retry the disposal motor automatically.",
    ...
    def retry_upload(self, job):
        ...
        disposal_failed_upload = str(job.get("status") or "") == "disposal_failed_upload_pending"

        if ok:
            if disposal_failed_upload:
                mark_job(
                    job_id,
                    "disposal_failed",
                    last_error=job.get("last_error") or "Trash disposal failed. Operator assistance is required.",
                )
                publish_queue_state(
                    "disposal_failed",
                    f"Upload completed for disposal-failed kit {job_id}",
                )
                print(
                    f"[KIT QUEUE] Upload completed for disposal-failed job={job_id}",
                    flush=True,
                )
            else:
                mark_job(job_id, "completed")
            ...
            mark_job(
                job_id,
                "disposal_failed_upload_pending" if disposal_failed_upload else "upload_pending",
            )
            publish_queue_state(
                "upload_pending",
                f"Upload retry in {retry_delay}s",
            )
